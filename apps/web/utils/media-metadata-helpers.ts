...
export async function generateMediaMetadata({
  ...
  if (!media) {
    return buildNotFoundMetadata(
      notFoundTitle ||
        `${mediaType === "movie" ? "Película" : "Serie"} No Encontrada`,
    );
  }
  ...
  const description = buildMediaDescription({
    ...
    fallback: `Ver ${title} en Cuevana 3`,
  });
  ...
  const path =
    canonicalPath ??
    (mediaType === "movie" ? `/peliculas/${mediaId}` : `/series/${mediaId}`);
  ...
  return buildPageMetadata({
    ...
    path,
    ogType: "website",
    imageAlt: `${titleWithYear} en Cuevana 3`,
