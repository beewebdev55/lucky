...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    description: collection.overview
      ...
      : `Browse the ${collection.name} movie collection on Cuevana 3.`,
    path: `/collection/${id}`,
    includeDefaultImage: false,
    imageAlt: `${collection.name} on Cuevana 3`,
  });
}
...
export default async function CollectionPage(props: Props) {
  ...
  const structuredData = {
    ...
    description: collection.overview,
    url: `https://cuevana33.lat/collection/${id}`,
      ...
      <ContentContainer className="relative z-10" topSpacing={false}>
        <div className="mx-auto w-full max-w-7xl space-y-6 px-2 pb-12 sm:px-4">
          <div className="flex flex-col gap-4 pb-8 pt-20 md:pt-28">
            <BackButton fallbackUrl="/peliculas" />
            <div>
