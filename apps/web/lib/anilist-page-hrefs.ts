...
/** Keep anime hub/grid links on Cuevana 3 — never outbound to AniList or non-anime /tvshows routes. */
export const withAnimePageHref = (item: MediaItem): MediaItem => {
  ...
  if (item.media_type === "movie") {
    ...
    if (animeItem.tmdbFallback?.type === "movie") {
      return {
        ...item,
        href: `/peliculas/${animeItem.tmdbFallback.id}`,
      } as MediaItem;
    }
  }
  ...
  if (animeItem.tmdbFallback) {
    ...
    return {
      ...item,
      href:
        type === "movie" ? `/peliculas/${id}` : buildTmdbAnimeDetailHref(id, "tv"),
    } as MediaItem;
  }
