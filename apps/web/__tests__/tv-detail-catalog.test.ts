...
describe("tv detail catalog", () => {
  ...
  it("resolves prefetch ids from /anime hrefs", () => {
    ...
    expect(resolveAnimePrefetchRouteId("/series/11757")).toBeNull();
  });

  it("resolves catalog from detail hrefs", () => {
    expect(resolveTvDetailCatalogFromHref("/anime/20021")).toBe("anime");
    expect(resolveTvDetailCatalogFromHref("/series/11757")).toBe("tvshows");
    expect(resolveTvDetailCatalogFromHref("/peliculas/550")).toBeNull();
  });
  ...
  it("picks /api/anime vs /api/tv from the page URL namespace", () => {
    ...
    expect(withPageTvApiPath("/api/tv/11757/season/2", "/series/11757")).toBe(
