...
export function getSearchResultTitle(result: SearchPreviewResult): string {
  return result.title || result.name || "Título desconocido";
}

function getFooterLabel(footer: SearchAutocompleteFooter): string {
  switch (footer) {
    case "go-to-search":
      return "Ir a la página de búsqueda";
    case "view-all":
      return "Mostrar todos los resultados";
    case "none":
      return "";
    default: {
...
export function SearchAutocomplete({
  ...
  const statusMessage = isLoading
    ? "Cargando resultados"
    : hasContent
      ? `${displayedResults.length} resultados disponibles. Usa las flechas para navegar.`
      : query.trim().length > 0
        ...
        {isLoading ? (
          ...
          <>
            {displayedResults.length > 0 && (
              <div role="group" aria-label="Títulos coincidentes" className="py-1">
...
export function getSearchComboboxInputProps({
  ...
  listboxId,
  activeOptionId,
  ariaLabel = "Buscar películas y series",
}: {
  isOpen: boolean;
  listboxId: string;
