...
export function MediaInfoDialog({
  ...
  const href = match(media)
    .with({ title: P.string, id: P.number }, (movie) => `/peliculas/${movie.id}`)
    .with({ name: P.string, id: P.number }, (tvShow) => `/series/${tvShow.id}`)
    .otherwise(() => "#");
    ...
    () =>
      match(current)
        .with(
          ...
          (movie) => `/peliculas/${movie.id}`,
        )
        .with(
          { name: P.string, id: P.number },
          (tvShow) => `/series/${tvShow.id}`,
        )
        .otherwise(() => "#"),
    [current],
  );
