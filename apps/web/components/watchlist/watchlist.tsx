...
import { getWatchlistItem } from "@/app/mi-lista/actions";
...
export function WatchlistButton({
  ...
  const handleToggle = async () => {
    ...
    if (!isSignedIn) {
      toast.error("Para agregar tÃ­tulos a tu lista, debes iniciar sesiÃ³n.", {
        action: {
    ...
    try {
      ...
      toast.error(
        isInWatchlist
          ? "Error al eliminar de tu lista"
          : "Error al agregar a tu lista",
      );
    } finally {
  ...
  return (
    <Button
      ...
      aria-label={isInWatchlist ? "Eliminar de tu lista" : "Agregar a tu lista"}
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div className="flex h-9 w-full rounded-md bg-black/35 p-1 md:w-auto">
              {["Todos", "PelÃ­culas", "Series"].map((label, index) => (
                <div
                  key={label}
                  className={cn(
    ...
    >
      {items.length === 0 ? (
        <div className="flex min-h-24 items-center justify-center gap-2 rounded-lg border border-dashed border-white/10 bg-black/20 px-4 py-6 text-center text-sm text-zinc-400">
          ...
          <span>{emptyDescription || "No hay tÃ­tulos en esta secciÃ³n aÃºn."}</span>
        </div>
      ) : (
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-medium text-zinc-200">
              ...
              >
                {isUnavailable ? "No disponible" : getMediaLabel(item)}
              </span>
              {isUnavailable ? (
