...
const MediaPeekBody = ({
  ...
  const [movieButtonLabel, setMovieButtonLabel] = useState<"Reproducir" | "Continuar">(
    "Reproducir",
  );
  ...
  const facts = useMemo(
    () =>
      [
        runtimeLabel ? { label: "Duración", value: runtimeLabel } : null,
        formatEndsAt(runtimeMinutes)
          ? { label: "Termina a las", value: formatEndsAt(runtimeMinutes) ?? "" }
          : null,
        language
          ? { label: "Idioma", value: format.country(language) }
          : null,
        releaseLabel ? { label: "Estreno", value: releaseLabel } : null,
        directorPerson
          ? {
              label: directorLabel,
              value: directorPerson.name,
              href: `/persona/${directorPerson.id}`,
            }
          : null,
      ].filter(
          ...
          <div className="space-y-8 bg-linear-to-b from-background via-background to-muted/10 px-4 py-7 md:space-y-10 md:px-8 md:py-9 md:pb-14">
            ...
            {cast.length > 0 ? (
              <section
                ...
                style={riseStyle(120)}
              >
                <MediaPeekSectionHeading title="Elenco" />
            ...
            {videos.length > 0 ? (
              <section
                ...
                style={riseStyle(160)}
              >
                <MediaPeekSectionHeading title="Tráileres y clips" />
              ...
              >
                <MediaPeekSectionHeading
                  ...
                  action={
                    ...
                    >
                      <Link href={target.href} onClick={handleCloseNavigate}>
                        Ver todo en la página de detalles
                      </Link>
                    </Button>
                  }
                />
